...
import uz.instat.fitneskittest.busines.enums.Errors.Companion.message
...
import uz.instat.fitneskittest.framework.MainViewModel
import uz.instat.fitneskittest.presentation.adapter.LessonAdapter
...
class LessonFragment : Fragment() {
    ...
    private val lessonObserver = Observer<UiState<List<LessonsHistoryLocal>>> {
        when (it) {
            is UiState.Loading -> {
            ...
            is UiState.Success -> {
                ...
                if (it.data != null && it.data.isNotEmpty()) {
                    ...
                    val lessons = arrayListOf<LessonsHistoryLocal>()
                    list.forEach { date ->
                        lessons.add(LessonsHistoryLocal(date = date))
